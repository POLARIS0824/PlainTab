...
- 定时刷新和手动「拉取一批」走增量追加：新图插在轮换指针之后（下一张立即是新图）。
- toplist 增量拉取先完全按用户设置的窗口取：第 1 页探测 `last_page`，再在 `WALLHAVEN_TOPLIST_MAX_PAGE` 以内随机取深页；**只有这一批一张新图都没拿到时**，才用最近 24 小时日榜兜底（`topRange=1d` 的临时配置副本，只作用于该次请求，不写回用户设置）。单次请求上限 4（`WALLHAVEN_BATCH_MAX_REQUESTS`）。其余排序没有日榜兜底。
- 用户设置的 `topRange` 是增量拉取和建池的主来源，日榜只在主来源翻不出新图时救场，不要让它变成默认来源。
- 到期判定以 `providers.wallhaven.state.lastAddedAt`（最近一次真正追加到新图的时间）为锚点。空批次不推进锚点，也不能当成刷新完成，否则榜单饱和时会一直空转到下一个间隔；锚点过期后按冷却时间重试，避免每个新标签页都打接口。老数据没有该字段时退回 `lastSuccessAt`。
- 自动刷新没拿到新图时不显示「已更新」提示；手动「拉取一批」必须明确反馈「没有新图片」。
